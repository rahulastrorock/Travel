...
const TravelGuide = require('./src/models/TravelGuide')
...
async function seedDatabase() {
  try {
    ...
    // Add user references to travel guides
    const guidesWithUsers = travelGuides.map((guide, index) => ({
      ...guide,
      createdBy: createdUsers[0]._id // Assign admin as creator
    }));

    // Insert travel guides
    await mongoose.model('TravelGuide').insertMany(guidesWithUsers);
